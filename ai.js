...
cat[0].n ? ' Strongest category: ' + esc(cat[0].name) : '',
'',
...
function adminHelp() {
return [
'Hi! I\'m your MOOD admin assistant. Ask me things like:',
'',
...
async function llmAnswer(role, text) {
  const sys = role === 'admin'
    ? 'You are the AI assistant for MOOD Coffee Shop & Bakery (Kigali, Rwanda). Staff ask about customers, orders, earnings, products, money and operations. Answer briefly and clearly, in the same language as the question. Prices are in Rwandan Francs (RWF). Do not use emojis or symbols in your answers. If you do not know something, say so honestly.'
    : 'You are the friendly AI assistant for MOOD Coffee Shop & Bakery (Kigali, Rwanda). Customers ask about the menu, prices, delivery, loyalty points, gift cards, reservations, opening hours and their orders. Answer briefly and warmly, in the same language as the question. Prices are in Rwandan Francs (RWF). Do not use emojis or symbols in your answers. If unsure about something, tell them to ask the staff.';
  if (llmConfigured) {
    try {
      return await llm.ask({ system: sys, user: text });
    } catch (e) {
      console.error('[ai] LLM call failed:', e && e.message ? e.message : e);
    }
  }
  const help = role === 'admin' ? adminHelp() : customerHelp();
  const note = role === 'admin'
    ? 'I could not look that up right now. Meanwhile, here is what I can help you with:\n'
    : 'I could not find an answer for that right now. Meanwhile, here is what I can help you with:\n';
  return note + help;
}
